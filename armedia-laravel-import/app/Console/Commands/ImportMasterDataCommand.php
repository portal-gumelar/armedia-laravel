...
class ImportMasterDataCommand extends Command
{
    ...
    protected function importProducts(): void
    {
        ...
        foreach ($this->rows($sheet, 3) as $row) {
            ...
            Product::updateOrCreate(
                ...
                [
                    'nama' => $this->v($row, 2),
                    'kapasitas_mbps' => (int) ($this->v($row, 4) ?? 0),
    ...
    protected function importCustomers(): void
    {
        ...
        foreach ($this->rows($sheet, 5) as $row) {
            ...
            $product = ($kode = $this->v($row, 12)) ? Product::firstWhere('kode', $kode) : null;
            $odp = ($kodeOdp = $this->v($row, 18)) ? Odp::firstWhere('kode_odp', $kodeOdp) : null;

            Customer::updateOrCreate(
                ['id_baru' => $idBaru],
                [
                    'id_lama' => $this->v($row, 3),
                    'nama' => $this->v($row, 4) ?? '(tanpa nama)',
                    'nik_ktp' => $this->v($row, 5),
                    'no_hp' => $this->v($row, 6),
                    'kec' => $this->v($row, 7),
                    'desa' => $this->v($row, 8),
                    'rw' => $this->v($row, 9),
                    ...
                    'sn' => $this->v($row, 17),
                    'odp_id' => $odp?->id,
                    'tgl_aktif' => $this->toDate($this->v($row, 19)),
                    'panjang_kabel' => $this->v($row, 20),
                    'pon_olt' => $this->v($row, 21),
                    'keterangan' => $this->v($row, 22),
                    'status' => $this->v($row, 23) ?? 'Aktif',
    ...
    protected function importDevices(): void
    {
        ...
        foreach ($this->rows($sheet, 9) as $row) {
            ...
            $customer = ($idPel = $this->v($row, 8)) ? Customer::firstWhere('id_baru', $idPel) : null;
    ...
    protected function importOntInventories(): void
    {
        ...
        if ($sheet = $this->sheet('12_ONT_NEW')) {
            foreach ($this->rows($sheet, 3) as $row) {
                ...
                $customer = ($idPel = $this->v($row, 15)) ? Customer::firstWhere('id_baru', $idPel) : null;
    ...
    protected function importNetwatch(): void
    {
        ...
        foreach ($this->rows($sheet, 4) as $row) {
            ...
            $customer = ($idPel = $this->v($row, 4)) ? Customer::firstWhere('id_baru', $idPel) : null;
    ...
    protected function importMonthlyBills(): void
    {
        ...
        foreach ($this->rows($sheet, 5) as $row) {
            ...
            $customer = Customer::firstWhere('id_baru', $idArm);
    ...
    protected function importProrata(): void
    {
        ...
        foreach ($this->rows($sheet, 5) as $row) {
            ...
            foreach ($productCols as $col => $namaProduk) {
                $product = Product::firstWhere('nama', $namaProduk);
